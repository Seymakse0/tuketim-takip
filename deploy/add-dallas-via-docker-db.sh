#!/usr/bin/env bash
# Host'ta npm/node_modules izni veya Prisma motoru olmadan Dallas steak satırını ekler.
# Önkoşul: docker compose ile db ayakta; proje kökünde .env (POSTGRES_PASSWORD).
# Kullanım: ./deploy/add-dallas-via-docker-db.sh
set -euo pipefail
...
SQL_FILE="$SCRIPT_DIR/sql/add-dallas-steak.sql"
...
echo "==> SQL uygulanıyor: add-dallas-steak.sql"
